...
from userbot.util import load_module, start_mybot, load_pmbot
...
print("AkenoXNew has been deployed! ")

print("Setting up TGBot")
path = "userbot/modules/mybot/*.py"
files = glob.glob(path)
for name in files:
    with open(name) as f:
        path1 = Path(f.name)
...
if LOAD_MYBOT == "True":
    path = "userbot/modules/mybot/pmbot/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as f:
            path1 = Path(f.name)
...
print("AkenoXNew has been fully deployed! Do Visit @OreBaka")
