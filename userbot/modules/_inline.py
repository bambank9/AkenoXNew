...
from userbot.modules import telestats
...
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`AkenoXNew PM security! Please wait for me to approve you. 😊"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "AkenoXNew User"
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("`Userbot"):
            ...
            result = builder.article(
                title="Stats",
                text=f"**AkenoXNew Stats For [{DEFAULTUSER}](tg://user?id={myid})**\n\n__Bot is functioning normally, master!__\n\n(c) @TeleBotSupport",
                buttons=[
                    [custom.Button.inline("Stats", data="statcheck")],
                    [Button.url("Repo", "https://github.com/rizgustiadi/AkenoXNew")],
                    [
                        Button.url(
                            "Deploy Now!",
                            "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Frizgustiadi%2FAkenoXNew&template=https%3A%2F%2Fgithub.com%2Frizgustiadi%2FAkenoXNew",
                        )
                    ],
                ],
            )
            ...
            result = builder.article(
                title="Repository",
                text=f"AkenoXNew - Telegram Userbot.",
                buttons=[
                    [
                        Button.url("Repo", "https://github.com/rizgustiadi/AkenoXNew"),
                        Button.url(
                    ...
                    [Button.url("Support", "https://t.me/OreBaka")],
                ],
            )
        else:
            result = builder.article(
                "Source Code",
                text="**Welcome to AkenoXNew**\n\n`Click below buttons for more`",
                buttons=[
                    [custom.Button.url("Creator👨‍🦱", "https://t.me/OreBaka")],
                    [
                        custom.Button.url(
                            "👨‍💻Source Code‍💻", "https://github.com/rizgustiadi/AkenoXNew"
                        ),
                        custom.Button.url(
                            "Deploy 🌀",
                            "https://dashboard.heroku.com/new?template=https%3A%2F%2Fgithub.com%2Frizgustiadi%2FAkenoXNew",
                        ),
                    ],
                    [
                        custom.Button.url(
                            "Support Me :v↗️", "https://t.me/AkenoXNew"
                        )
                    ],
                ],
                link_preview=False,
